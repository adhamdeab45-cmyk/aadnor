(function(){
  ...
  function mergeLottery(v){const out=JSON.parse(JSON.stringify(defaultLottery));if(!v)return out;Object.assign(out,v);drawOrder.forEach(k=>out[k]=Object.assign({},defaultLottery[k],v[k]||{}));return out}
  ...
  function functionError(e){
    ...
    const map={'unauthenticated':'سجّل الدخول أولاً.','permission-denied':'ليس لديك صلاحية لهذه العملية.','failed-precondition':'العملية غير متاحة حالياً.','resource-exhausted':'تم تجاوز الحد المسموح.','invalid-argument':'تحقق من البيانات المدخلة.','not-found':'العنصر المطلوب غير موجود.','already-exists':'تم تنفيذ هذه العملية سابقاً.','unavailable':'السيرفر غير متاح. انشر Cloud Functions أولاً.','internal':'حدث خطأ في السيرفر.'};
    return e?.message?.includes('Failed to fetch')?'Cloud Functions غير منشورة أو غير متاحة. راجع ملف README_AR.md.':(map[code]||e?.message||'تعذر تنفيذ العملية.');
  }
  async function call(name,data={},timeout=20000){
    const fn=functions.httpsCallable(name);let timer;
    try{return await Promise.race([fn(data),new Promise((_,rej)=>timer=setTimeout(()=>rej(new Error('انتهت مهلة الاتصال بالسيرفر.')),timeout))])}
  ...
  async function checkAppHealth(){if(!state.user||state.settings.maintenanceEnabled)return;try{const r=await call('healthCheck',{},8000);$('systemStatusText').textContent=r.data?.ok?'ONLINE':'ERROR';$('systemStatusText').dataset.state=r.data?.ok?'online':'error'}catch(e){$('systemStatusText').textContent=navigator.onLine?'SERVER ERROR':'OFFLINE';$('systemStatusText').dataset.state='error'}}
  ...
  async function onSignedIn(user){
    state.user=user;authView(true);if(!(await ensureProfile()))return;listenPrivate();const token=await user.getIdTokenResult(true).catch(()=>null);$('adminLink').classList.toggle('hidden',!token?.claims?.admin);page('home');track('login_success',{provider:user.providerData?.[0]?.providerId||'unknown'});checkAppHealth();clearInterval(state.healthTimer);state.healthTimer=setInterval(checkAppHealth,60000);
  }
